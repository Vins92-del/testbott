...
token = "1797322941:AAE5oqKfu4Y54QSOS-ZkSPgqgW5sVRPAaqg"
...
def handle(update):
	...
	try:
		...
		if update["text"].split()[0] == "/pw":
			...
			keyboard = ReplyKeyboardMarkup(keyboard=[['Cari Pasangan Sange👥', '🔙 Main Menu']], resize_keyboard=True)
		...
		if len(queue["free"]) > 1 and not uid in queue["occupied"]:
			...
			if partner != uid:
				...
				bot.sendMessage(uid, 'Pasangan sange ditemukan.., selamat ah aha ah😜', reply_markup=keyboard)
				bot.sendMessage(partner, 'Pasangan sange ditemukann.., selamat sange😜', reply_markup=keyboard)
